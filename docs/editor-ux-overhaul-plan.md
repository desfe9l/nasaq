...
| 7 | Tablet toolbar 768–1024px: no multi-row wrapping | done | Toolbar `md:flex-nowrap md:overflow-x-auto md:whitespace-nowrap`; tool tray `md:min-w-0`, draw-text label collapses to its icon below `lg`. In that pass the phone kept `min-w-fit` + `order-last` and a second line; the sixth work order later replaced that second line with one scrolling row at every width |
...
- [x] Top toolbar never wraps at **any** width: the row is `flex-nowrap` and is itself the horizontal scroller (`overflow-x-auto` + `whitespace-nowrap`), while the brand group, the history + zoom cluster and the actions group are all `shrink-0` — so nothing is squeezed, clipped off the edge, or stacked into ragged lines. At `md` and up the tool tray is `min-w-0` and takes the slack: it scrolls inside its own box, the row itself does not scroll, and Save/Export plus Undo/Redo/Zoom/Fit stay pinned at the row's two edges. On a phone the four groups together are wider than the screen, so the row scrolls as one strip: Undo/Redo/Zoom/Fit follow the brand group immediately and Save/Export right after them (the tray is `order-last` there), i.e. at most one short swipe away down to 320 px.
