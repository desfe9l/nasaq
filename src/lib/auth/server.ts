 ...
 *
 * Pre-wired for live preview + deploy — Google is the only social provider.
 * To enable local email/password, flip the flag in `./email-password` only.
 *
 * The app runs its own Better Auth at `/api/auth/*`, so the session cookie stays
 * on this app's own origin. Sign-in uses Better Auth's native Google provider
...
// schema from the root `migrations/0003_auth.sql` migration.
...
const googleSocialProvider = authConfigured
  ? {
      google: {
