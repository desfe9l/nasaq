...
test("shape-affine: identity keeps rectangles and polygons in place", () => {
  assert.deepEqual(
    mapShapePart({ k: "rect", x: 10, y: 20, w: 30, h: 40, rx: 4 }, IDENTITY),
    {
      k: "rect",
      x: 10,
      y: 20,
  ...
  assert.deepEqual(
    mapShapePart({ k: "poly", points: "0,0 50,100 100,0" }, IDENTITY),
    {
      k: "poly",
...
test("shape-affine: scale and translation land on the expected coordinates", () => {
  ...
  assert.deepEqual(mapShapePart({ k: "poly", points: "0,0 100,100" }, m), {
    k: "poly",
    points: "10,5 60,30",
  });
});
...
test("shape-affine: an unevenly scaled circle becomes an ellipse", () => {
  const even = mapShapePart(
    { k: "circle", cx: 50, cy: 50, r: 25 },
    { sx: 2, sy: 2, tx: 1, ty: 1 },
  );
  assert.deepEqual(even, { k: "circle", cx: 101, cy: 101, r: 50 });
  const uneven = mapShapePart(
    { k: "circle", cx: 50, cy: 50, r: 25 },
    { sx: 2, sy: 0.5, tx: 0, ty: 0 },
  );
...
test("shape-affine: arcs become curves whose midpoints stay on the ellipse", () => {
  ...
  assert.ok(
    !/[Aa]/.test(mapped),
    "no arc command may survive a non-uniform map",
  );
  const v = nums(mapped);
  ...
  assert.ok(
    Math.abs(v[6] - 110) < 0.5,
    `quarter point x should map to 110, got ${v[6]}`,
  );
  const mappedQuarterY = (v[7] - 20) / 3;
  assert.ok(
    Math.abs(Math.abs(mappedQuarterY) - 50) < 0.5,
    `quarter point y should map to ±50, got ${mappedQuarterY}`,
  );
});
    ...
    for (const part of def.parts) {
      ...
      if (mapped.k === "path") {
        ...
        for (const n of nums(mapped.d)) {
          assert.ok(
            Number.isFinite(n),
            `${id} produced a non-finite coordinate`,
          );
          assert.ok(
            n > -1 && n < 3,
            `${id} mapped a coordinate out of range: ${n}`,
          );
        }
      }
      if (mapped.k === "poly") {
        for (const pair of mapped.points.split(/\s+/)) {
          ...
          assert.ok(
            x > -1 && x < 3 && y > -1 && y < 3,
            `${id} polygon point out of range: ${pair}`,
          );
        }
      }
    }
