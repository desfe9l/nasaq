...
const ENV_SPECS: EnvSpec[] = [
  ...
  {
    key: "PAYLINK_API_BASE_URL",
    section: "payments",
    service: "Paylink API",
    account: "NASAQ production merchant",
    label: "Paylink API base URL",
    sensitivity: "public",
    purpose: "تحديد بيئة Paylink المستخدمة للخادم.",
    loginUrl: "https://my.paylink.sa",
    dashboardUrl: "https://my.paylink.sa",
    apiUrl: "https://restapi.paylink.sa",
    configurationLocation: "Vercel environment؛ server-only في src/lib/paylink/server.ts",
    fallbackValue: "https://restapi.paylink.sa",
    guide: guide("اختر restapi للإنتاج أو restpilot للاختبار، ثم أعد النشر.", "https://my.paylink.sa", "PAYLINK_API_BASE_URL"),
  },
  {
    key: "PAYLINK_API_ID",
    section: "payments",
    service: "Paylink API",
    account: "NASAQ production merchant",
    label: "Paylink API ID",
    sensitivity: "secret",
    purpose: "مفتاح مصادقة خادم Paylink لإنشاء الفواتير فقط.",
    loginUrl: "https://my.paylink.sa",
    dashboardUrl: "https://my.paylink.sa",
    apiUrl: "https://restapi.paylink.sa",
    configurationLocation: "Vercel environment؛ server-only في src/lib/paylink/server.ts",
    guide: guide("انسخ API ID من Paylink واحفظه في Vercel فقط.", "https://my.paylink.sa", "PAYLINK_API_ID"),
  },
  {
    key: "PAYLINK_SECRET_KEY",
    section: "payments",
    service: "Paylink API",
    account: "NASAQ production merchant",
    label: "Paylink Secret Key",
    sensitivity: "secret",
    purpose: "مفتاح مصادقة Paylink اللازم لاستدعاء API من الخادم.",
    loginUrl: "https://my.paylink.sa",
    dashboardUrl: "https://my.paylink.sa",
    apiUrl: "https://restapi.paylink.sa",
    configurationLocation: "Vercel environment؛ server-only في src/lib/paylink/server.ts",
    guide: guide("انسخ Secret Key من Paylink واحفظه في Vercel فقط ثم أعد النشر.", "https://my.paylink.sa", "PAYLINK_SECRET_KEY"),
  },
  {
    key: "PAYLINK_WEBHOOK_TOKEN",
    section: "webhooks",
    service: "Paylink Webhook",
    account: "NASAQ production merchant",
    label: "Paylink webhook bearer token",
    sensitivity: "secret",
    purpose: "قيمة Authorization التي يرسلها Paylink للتحقق من Webhook V2.",
    loginUrl: "https://my.paylink.sa",
    dashboardUrl: "https://my.paylink.sa",
    apiUrl: "https://developer.paylink.sa/docs/payment-webhook",
    configurationLocation: "Vercel environment؛ endpoint /api/webhooks/paylink",
    guide: guide("أنشئ Bearer token في Paylink، ثم أضفه إلى Vercel وLink webhook endpoint مع اختيار V2.", "https://my.paylink.sa", "PAYLINK_WEBHOOK_TOKEN", "Production", "نعم، إعادة نشر مطلوبة.", "Paylink → Settings → Webhooks → API version V2"),
  },
  {
    key: "PAYLINK_PUBLIC_URL",
    section: "deployment",
    service: "Paylink callbacks",
    account: "NASAQ production",
    label: "Paylink public application URL",
    sensitivity: "public",
    purpose: "بناء روابط النجاح والإلغاء التي يستخدمها Paylink.",
    loginUrl: "https://vercel.com/desfe9l/nasaq",
    dashboardUrl: "https://vercel.com/desfe9l/nasaq",
    apiUrl: null,
    configurationLocation: "Vercel environment؛ server-only في src/lib/paylink/server.ts",
    exampleValue: "https://nasaq-sa.vercel.app",
    guide: guide("حدد رابط NASAQ العام HTTPS الذي يطابق callback URL في Paylink.", "https://vercel.com/desfe9l/nasaq", "PAYLINK_PUBLIC_URL"),
  },
  {
    ...
    label: "Gumroad OAuth access token",
    sensitivity: "secret",
    ...
    guide: guide("أنشئ OAuth Application في Gumroad (Settings → Advanced → Application form) ثم اضغط Generate access token وأضفه في Vercel فقط — لا ترسله في الدردشة.", "https://gumroad.com/settings/advanced", "GUMROAD_ACCESS_TOKEN", "Production", "نعم، إعادة نشر مطلوبة.", "Gumroad Ping/Resource subscriptions على endpoint /api/webhooks/gumroad", "ألغِ token القديم بعد نجاح التحقق."),
  },
  {
  ...
  },
  {
    ...
    sensitivity: "public",
    purpose: "Policy اشتراك NASAQ الفردي السنوي عبر Paylink.",
    loginUrl: keygenDashboard,
    dashboardUrl: keygenDashboard,
    apiUrl: keygenApi,
    configurationLocation: "Vercel environment؛ server-only في src/lib/license/keygen.ts",
  ...
  },
  {
    ...
    sensitivity: "public",
    purpose: "Policy اشتراك NASAQ للفرق السنوي عبر Paylink.",
    loginUrl: keygenDashboard,
    dashboardUrl: keygenDashboard,
    apiUrl: keygenApi,
    configurationLocation: "Vercel environment؛ server-only في src/lib/license/keygen.ts",
...
const ROUTES: OwnerRouteInfo[] = [
  ...
  { label: "الشراء", path: "/purchase", status: "active", purpose: "إنشاء فواتير الدفع عبر Paylink." },
  { label: "Paylink webhook", path: "/api/webhooks/paylink", status: "active", purpose: "استقبال Payment Webhook V2 وتفعيل التراخيص." },
...
function serviceEntries(): VaultEntry[] {
  ...
  return [
    ...
    staticEntry({
      ...
      dashboardUrl: vercelDashboard,
      apiUrl: `${productionUrl}/api/webhooks/paylink`,
