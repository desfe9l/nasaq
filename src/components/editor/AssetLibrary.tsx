...
export function AssetLibrary() {
  ...
  /**
   * Insert a saved asset into the active artboard (step 10).
   *
   * A click centres it on what the author is looking at; dragging it onto the
   * canvas is handled by the shared library drop path, which places it under the
   * pointer. Both end with the same confirmation toast, so an insertion is never
   * silent.
   */
  ...
  /** Same payload the template cards send, so the canvas drop path is shared. */
      ...
      <header className="flex items-center justify-between gap-2">
        <div>
          ...
          <p className="mt-0.5 text-[10px] text-muted">
            معاينة قبل الحفظ، ثم إدراج وتعديل مباشر
          </p>
        </div>
        <div className="flex items-center gap-1">
          <button
            ...
            title="تصدير المكتبة (ملف واحد بكل المجلدات والعناصر)"
          ...
          <button
            ...
            title="استيراد مكتبة من ملف (يُدمج مع الحالي)"
            className="grid size-7 place-items-center rounded-[6px] border border-line dark:border-white/10"
          >
      ...
      {pending.length > 0 && (
        <div className="rounded-[8px] border border-gold/60 bg-gold/2 p-2 dark:bg-gold/10">
          <div className="mb-2 flex items-center justify-between gap-2">
            <div>
              ...
              <p className="text-[9px] text-muted">
                {pending.length} ملف جاهز للمراجعة
              </p>
            </div>
            <button
              type="button"
              onClick={() => setPending([])}
              className="grid size-6 place-items-center rounded-[6px] text-muted"
              title="إلغاء الملفات"
              aria-label="إلغاء الملفات"
            >
          ...
          >
            ...
            {savingPending ? "جارٍ الحفظ…" : "حفظ الملفات في المكتبة"}
          </button>
        </div>
      )}
      ...
      {assetsLoading ? (
        ...
        <div className="rounded-[8px] border border-dashed border-line p-3 text-center dark:border-white/10">
          <p className="text-[10px] leading-5 text-muted">
            احفظ أي صورة أو شعار أو شكل ترفعه ليظهر هنا وتستخدمه في أي مشروع
            لاحقاً.
          </p>
        </div>
      ) : (
        ...
        >
          {visibleAssets.map((asset) => (
            <div
              key={asset.id}
              /*
               * Draggable card (step 10): the drag carries the shared library
               * payload, so the canvas places the asset exactly under the drop
               * point instead of at a fixed corner.
               */
            ...
            >
              ...
              <button
                ...
                aria-haspopup="menu"
                className="absolute left-2 top-2 z-[2] grid size-5 place-items-center rounded-full border border-line bg-white/90 text-muted dark:border-white/20 dark:bg-[#161c26]/90"
                title="الخيارات"
              >
                ...
                <>
                  ...
                  {!folders.length && (
                    <p className="px-2.5 py-1.5 text-[10px] leading-5 text-muted">
                      لا توجد مجلدات بعد — أنشئ مجلدًا من شريط المجلدات أعلى
                      المكتبة.
                    </p>
                  )}
                </>
      ...
      {preview &&
        createPortal(
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={`معاينة ${preview.name}`}
            onClick={() => setPreview(null)}
          >
            <div
              className="w-full max-w-sm rounded-[10px] bg-white p-3 shadow-xl dark:bg-[#161c26]"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="mb-3 flex items-center justify-between gap-2">
                <div>
                  <p className="text-[12px] font-extrabold">معاينة العنصر</p>
                  <p className="max-w-[15rem] truncate text-[10px] text-muted">
                    {preview.name}
                  </p>
                </div>
                <button
            ...
            {folderDialog === "delete" ? (
              <div className="grid w-full max-w-xs gap-3 rounded-[10px] bg-white p-4 shadow-xl dark:bg-[#161c26]">
                ...
                <p className="text-[11px] leading-6 text-muted">
                  سيتم حذف هذا المجلد ومحتوياته نهائيًا، ولا يمكن التراجع عن هذا
                  الإجراء.
                </p>
                <div className="flex justify-end gap-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (currentFolder)
                        void deleteAssetFolder(currentFolder.id);
              ...
              <form
                ...
                onSubmit={(event) => {
                  ...
                  if (folderDialog === "create")
                    void createAssetFolder(folderDraft);
                  else if (currentFolder)
                    void renameAssetFolder(currentFolder.id, folderDraft);
                  setFolderDialog(null);
                }}
              >
                ...
                <input
                  ...
                  className="h-9 rounded-[7px] border border-line px-2 text-[12px] dark:border-white/10 dark:bg-white/5"
                />
                <div className="flex justify-end gap-2">
                  <button
                    type="button"
      ...
      {assetToDelete &&
        createPortal(
          ...
          <div
            ...
            onKeyDown={(event) => {
              if (event.key === "Escape") setAssetToDelete(null);
            }}
          >
            <div className="grid w-full max-w-xs gap-3 rounded-[10px] bg-white p-4 shadow-xl dark:bg-[#161c26]">
              <strong className="text-[13px]">هل أنت متأكد من الحذف؟</strong>
              <p className="text-[11px] leading-6 text-muted">
                سيتم حذف «{assetToDelete.name}» من المكتبة نهائيًا. العناصر التي
                أُدرجت في الصفحات لا تتأثر.
              </p>
              <div className="flex justify-end gap-2">
                <button
                  type="button"
...
function imageDimensions(src: string): Promise<{ w: number; h: number }> {
  return new Promise((resolve) => {
    ...
    image.onload = () =>
      resolve({ w: image.naturalWidth || 100, h: image.naturalHeight || 100 });
